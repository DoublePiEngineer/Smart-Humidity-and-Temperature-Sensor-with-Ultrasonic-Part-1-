...
#define DHTPIN 2     // what digital pin we're connected to sensor
...
// defines pins numbers of ultrasonic HC-SR04
const int trigPin = 6;
const int echoPin = 5;
...
void loop(void) {
  ...
  u8g2.drawStr(9,44,"Celsius");	// write something to the internal memory
  ...
  u8g2.clearBuffer();          // clear the internal memory
  u8g2.setFont( u8g2_font_logisoso26_tf );  // choose a suitable font
  u8g2.drawStr(3,44,"Humidity"); // write something to the internal memory
  u8g2.sendBuffer();          // transfer internal memory to the display
  delay(2000);  
